...
\begin{itemize}
\item une soutenance orale de 15 minutes (avec supports Power-Point (ou Beamer/\LaTeX) le \textbf{17 Janvier 2016}(ROB) ou le \textbf{?????}(MAIN)
\item un tournoi de Labyrinthe entre les binomes. Le gagnant du tournoi aura l'honneur de rencontrer un programme réalisé par un enseignant et invaincu (par définition) jusqu'à aujourd'hui. Les modalités détaillées de la soutenance seront précisées début décembre. Le tournoi sera commun entre les spécialités ROB et MAIN.
\end{itemize}
...
est aléatoire telle que $L$ et $H$ soient impairs, $13\leq L\leq 17$ et $9\leq H\leq 15$
...
Ensuite, à chaque tour de jeu, vous pouvez soit effectuer un déplacement classique de votre joueur, soit (à certaines conditions) effectuer une rotation du labyrinthe.
...
A chaque tour vous pouvez vous déplacer d'une case vers une case libre (c'est à dire sans mur) vers 
...
\begin{itemize}
\item A chaque fois que vous faites un déplacement de ce type, vous gagnez une unité d'énergie.
...
\item Vous ne respectez pas le séquencement du programme (cf section~\label{sec:sequences})
...
\begin{itemize}
\item Copier sur votre compte les templates \verb|template.c| et \verb|makefile| fournis que vous pourrez modifier (tous le contenu du répertoire \verb|src|)\\
...
\subsection{Etapes à respecter dans votre programme}
\label{sec:sequences}
...
\begin{itemize}
\item Si c'est à vous de jouer : Envoyer le votre coup en appelant la fonction \verb|sendMove|, vous récupérez le code \verb|ret|.
\item Si c'est à l'adversaire de jouer : Récpérer le coup jouer par l'adversaire en appelant la fonction \verb|getMove|, vous récupérez égalelemnt le code \verb|ret|.
...
\end{itemize}
\end{itemize}
\item Ferme la connexion au serveur en appelant la fonction \verb|closeConnection|
\end{enumerate}
De plus, à tout moment, vous pouvez demander l'affiche du jeu à l'écran en appelant la fonction \verb|printLabyrinth|.
...
Pour cela, il faut spécifier dans une chaîne \verb|training| de caractère le nom de ce joueur par défaut, ainsi que les paramètres. Pour l'instant, deux joueurs par défauts sont disponibles : 
\begin{itemize}
...
\item \textbf{PLAY\_RANDOM} : Ce joueur joue aléatoirement, mais tous ses coups sont autorisés.

Exemple : \verb|training = "PLAY_RANDOM timeout=60 rotation=True"| pour joueur avec 60 s de limite de temps et la possibilité pour le joueur PLAY\_RANDOM de faire les rortations de lignes et colonnes dans le labyrinthe (mettre l'option à \verb|False| si on veut que PLAY\_RANDOM ne fasse que des déplacements classiques.
...
Le numéro de port et le nom du serveur vous seront donné ultérieurement.
...
\label{sec:wait}
\lstinputlisting[language=C, firstline=79, lastline=106]{../API/labyrinthAPI.h}
...
la valeur sera donc données en sortie de fonction.
